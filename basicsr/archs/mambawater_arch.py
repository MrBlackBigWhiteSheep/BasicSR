...
# ==============================================================================
# 1. 物理流形感知网络 (Robust Version)
#    修复点：防止 Max/Min 产生 Outlier，增加 Tanh 压缩
# ==============================================================================
class PhysicalManifoldNet(nn.Module):
    def __init__(self, dim, reduction=4):
        super().__init__()
        # [修复 4] 增加一个 LayerNorm 在统计量拼接之前/之后，或者使用 Tanh
        self.input_proj = nn.Conv2d(dim * 3, dim // reduction, 1)
        
        self.fusion = nn.Sequential(
            nn.InstanceNorm2d(dim // reduction, eps=1e-5), # 保持 InstanceNorm 去除绝对亮度
            nn.Tanh(), # [核心修复 4] 强制将特征压缩到 [-1, 1]，防止 Outlier 传导
            nn.Conv2d(dim // reduction, dim, kernel_size=3, padding=1, groups=dim // reduction),
            nn.Sigmoid() 
        )
        self.out_proj = nn.Conv2d(dim, 1, 1)

    def forward(self, x):
        # 计算 Dark/Bright Channel Prior
        max_c, _ = torch.max(x, dim=1, keepdim=True)
        min_c, _ = torch.min(x, dim=1, keepdim=True)
        
        # 拼接
        x_stat = torch.cat([x, max_c.expand_as(x), min_c.expand_as(x)], dim=1)
        
        # 投影 + 非线性压缩
        feat = self.input_proj(x_stat)
        
        # 输出 P Map
        return self.out_proj(self.fusion(feat))

# ==============================================================================
# 2. ASSM: Attention-based State Space Module (The "Textbook" Stable Version)
#    修复点：有界 Barrier、梯度截断排序、归一化融合
# ==============================================================================
class ASSM(nn.Module):
    def __init__(self, dim, d_state=16, input_resolution=None, num_tokens=None, mlp_ratio=2., **kwargs):
        super().__init__()
        self.dim = dim
        ...
        self.local_context = nn.Conv2d(self.d_inner, self.d_inner, kernel_size=3, padding=1, groups=self.d_inner, bias=True)
        self.act = nn.SiLU()
        ...
        # Stability Modules
        self.prior_net = PhysicalManifoldNet(dim)
        self.barrier_proj = nn.Linear(1, self.d_inner)
        self.barrier_scale = nn.Parameter(torch.tensor(0.1))
        
        self.norm_fwd = nn.LayerNorm(self.d_inner)
        self.norm_rev = nn.LayerNorm(self.d_inner)
    ...
    def _init_weights(self):
        # 1. 屏障初始化为0
        nn.init.constant_(self.barrier_proj.weight, 0)
        nn.init.constant_(self.barrier_proj.bias, 0)
        
        # 2. 【至关重要】dt_proj 初始化
        # Mamba 对 dt 的初始化极其敏感，随机初始化会导致开局 dt 很大或很小，直接 NaN
        dt_init_std = self.dt_rank**-0.5 * 0.1
        nn.init.uniform_(self.dt_proj.weight, -dt_init_std, dt_init_std)
        
        # 初始化 bias 使得 dt 在 softplus 后大约为 0.001~0.1 之间
        # inv_softplus(0.1) ≈ -2.3
        nn.init.constant_(self.dt_proj.bias, -2.3) 
    ...
    def forward(self, x, x_size, token=None):
        ...
        x_2d = x_res.transpose(1, 2).view(B, self.d_inner, H, W)
        x_local = self.act(self.local_context(x_2d)).flatten(2).transpose(1, 2)

        # 2. Physical Manifold
        x_img = x.transpose(1, 2).view(B, C, H, W)
        P_map = self.prior_net(x_img) 
        P_flat = P_map.flatten(2).transpose(1, 2)
        P_for_sort = P_flat.detach()
        
        P_sorted_values, sort_idx = torch.sort(P_for_sort, dim=1) 
        undo_sort_idx = torch.argsort(sort_idx, dim=1)

        x_sorted = torch.gather(x_local, 1, sort_idx.expand(-1, -1, self.d_inner))
        P_sorted_grad = torch.gather(P_flat, 1, sort_idx.expand(-1, -1, 1))

        # 3. Dynamic Barrier
        P_diff = torch.zeros_like(P_sorted_grad)
        P_diff[:, 1:, :] = P_sorted_grad[:, 1:, :] - P_sorted_grad[:, :-1, :]
        barrier_signal = torch.tanh(self.barrier_proj(P_diff)) * self.barrier_scale

        # 4. Scan Parameters
        x_dbl = self.x_proj(x_sorted)
        dt_rank, B_ssm, C_ssm = torch.split(x_dbl, [self.dt_proj.in_features, self.d_state, self.d_state], dim=-1)
        
        # 计算 raw dt
        dt = self.dt_proj(dt_rank) + barrier_signal
        
        # ========================================================
        # 【核心修复 1】: Softplus 激活 (防止负时间导致的爆炸)
        # ========================================================
        dt = F.softplus(dt) 

        # FP32 锁
        if x_sorted.dtype == torch.float16:
            x_sorted = x_sorted.float()
            dt = dt.float()
            B_ssm = B_ssm.float()
            C_ssm = C_ssm.float()
        ...
        y_fwd = selective_scan_fn(
            x_sorted.transpose(1, 2), dt.transpose(1, 2), A, 
            B_ssm.transpose(1, 2), C_ssm.transpose(1, 2), D, 
            z=None, return_last_state=None
        )

        # Backward Scan
        x_rev = torch.flip(x_sorted, dims=[1])
        dt_rev = torch.flip(dt, dims=[1])
        B_rev = torch.flip(B_ssm, dims=[1])
        C_rev = torch.flip(C_ssm, dims=[1])
        
        y_rev = selective_scan_fn(
            x_rev.transpose(1, 2), dt_rev.transpose(1, 2), A, 
            B_rev.transpose(1, 2), C_rev.transpose(1, 2), D, 
            z=None, return_last_state=None
        )
        y_rev = torch.flip(y_rev, dims=[2])

        # 归一化融合
        y_fwd_norm = self.norm_fwd(y_fwd.transpose(1, 2)).transpose(1, 2)
        y_rev_norm = self.norm_rev(y_rev.transpose(1, 2)).transpose(1, 2)
        
        P_weight = P_sorted_grad.transpose(1, 2)
        y_merged = y_fwd_norm * P_weight + y_rev_norm * (1.0 - P_weight)

        if x.dtype == torch.float16:
            y_merged = y_merged.half()

        y_restored = torch.gather(y_merged.transpose(1, 2), 1, undo_sort_idx.expand(-1, -1, self.d_inner))
        out = y_restored * self.act(z)
